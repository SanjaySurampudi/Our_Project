...
def _autodetect_port():
    """
    Return the serial device of the first port whose description looks
    like an Arduino / USB-serial adapter, or None if nothing matches.
    """
    keywords = ['Arduino', 'CH340', 'CP210', 'FTDI', 'USB Serial', 'wchusbserial', 'ttyUSB', 'ttyACM']
...
def _resolve_port(port_override=None):
    """
    Resolve which serial port to open, in priority order:
        1. explicit `port_override` argument (used by tests)
        2. LORA_PORT environment variable
        3. auto-detection by USB descriptor keyword
    Returns the chosen port string, or None when nothing was found.
    """
    if port_override:
...
def _list_available_ports():
    """Return a human-readable list of currently visible serial ports."""
    ports = list(serial.tools.list_ports.comports())
    if not ports:
        return "  (no serial devices currently visible to the OS)"
    return "\n".join(f"  - {p.device}  ({p.description or 'no description'})" for p in ports)
...
def read_serial(port_override=None):
    ...
    if PORT is None:
        ...
        print("  No port matched (Arduino / CH340 / CP210 / FTDI / USB Serial / ttyUSB / ttyACM).")
        print()
        print("  Available serial ports right now:")
        print(_list_available_ports())
        print()
        print("  Fix one of these and restart:")
        print("    1. Plug in the Arduino and try again, OR")
        print("    2. Set LORA_PORT environment variable, e.g.")
        print("         Linux/macOS:  export LORA_PORT=/dev/ttyUSB0")
        print("         Windows:      set LORA_PORT=COM11")
        print()
    ...
    while True:
        try:
            ...
            print(f"Serial error on {PORT}: {e} - retry in 4s (attempt {consecutive_failures})")
            # After 5 failures in a row, re-run auto-detect in case the
            # user just plugged the cable into a different port.
...
if __name__ == '__main__':
    ...
    # Bind to 127.0.0.1 (localhost only) by default for security.
    # Set FLASK_HOST=0.0.0.0 for LAN access (e.g., from phone on same WiFi).
    host = os.environ.get('FLASK_HOST', '127.0.0.1')
    if host != '127.0.0.1':
        print(f"  WARNING: Binding to {host} - dashboard accessible on LAN")
        print("  GPS data and message content exposed without authentication")
        print("=" * 50)
